...
class ProfessorModel(db.Model):
    ...
    def json(self):
        return {'id': self.id, 'name': self.name, 'email': self.email, 'university_id': self.university_id, 'location': self.location, 'image_link': self.image_link, 'location_id': self.location_id}