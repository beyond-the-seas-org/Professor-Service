...
class Get_locationbased_professors(Resource):
    @api.doc(responses={200: 'OK', 404: 'Not Found', 500: 'Internal Server Error'})

    def get(self, location_id):

        try:
            #get all professors where ProfessorModel.location_id is location_id with sorted by university rank
            all_professors_short_details = db.session.query(ProfessorModel, UniversityRankModel).join(UniversityRankModel, ProfessorModel.university_id == UniversityRankModel.id).filter(ProfessorModel.location_id == location_id).order_by(UniversityRankModel.rank).all()
            ...
            for professor in all_professors_short_details:
                ...
                all_professors_short_details_json.append({
                    ...
                    "location_id": professor[0].location_id,
